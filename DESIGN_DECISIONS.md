...
## 41. Evaluation harness Phase 4 fixes: COMET cache, bootstrap CIs, fingerprints, seeded MBR, length/TER, ref-clean, entity panel

**Date:** 2026-08-03

**Context:** An independent deep-read of the eval harness found the reported numbers rest on unsound mechanics. `comet_score.py` keyed its cache on (tag, suite) only, so regenerating a hyp file under an existing tag silently reported the old score forever; `metrics_mt.py` computed sacreBLEU BLEU()/CHRF() and discarded the bootstrap CI objects, so "DoRA = equal quality (+/-0.5 chrF++)" and "B' edges A2 (+0.002 COMET)" are point estimates with no CIs; reports carried no fingerprint (model/adapter/beam/vocab/device/dtype), so resume keyed only on (en_text, hi_text) could mix stale rows and `--score-only` ignored `--max-pairs`; MBR N=8 sampling had no seed; verbosity is unpenalized (I_test hyps 1.377x ref length, BLEU `bp:1.0`, chrF++ has no length penalty); and reference noise (stray danda, list-marker corruption, OCR typos) caps every score with no ref-cleaned column and no reference-free QE. Harness code + tests + docs only -- no hyp file was re-decoded or re-scored, and no historical score table was edited.

**Decisions (formulas are as implemented):**

1. **COMET cache invalidation (P0-1)**. Cache key is now (tag, suite, hyp-file SHA256 prefix, model_id). `comet22_summary.json` is rewritten under a `schema: "v2"` header; pre-v2 payloads are ignored wholesale, so old entries never mask a re-score. `should_rescore(entry, fingerprint, model_id)` re-scores on missing/None score, fingerprint drift, or model change. `fingerprint_hyp_file` = first 16 hex chars of SHA256 of file bytes (content-based, not mtime).
2. **Bootstrap CIs (P0-2)**. `corpus_bleu`/`corpus_chrf` accept `n_bootstrap` and use sacreBLEU's built-in bootstrap (`corpus_score(..., n_bootstrap=N)`), which stores mean + 95% CI on the result object; `SACREBLEU_SEED` (default 12345) makes it reproducible. Reported per metric: `mean`, `ci_low = mean - ci`, `ci_high = mean + ci`, half-width `ci`, `n_bootstrap`, `seed`. Primary `score`/`signature`/`bp`/`sys_len`/`ref_len` semantics unchanged; the signature gains `bs:N|seed:S` fields. Per-suite `confidence` block summarizes score + mean + CI. The **difference** helper `paired_ci(hyps_a, hyps_b, refs, metric, n_resamples, seed)` uses sacreBLEU's own per-segment statistics extractor and `_compute_score_from_stats` (the same machinery its bootstrap uses, not a reimplemented metric), resamples segment indices with a seeded numpy RNG, recomputes the corpus-level score for both systems per resample, and reports `delta` (full-corpus difference), `mean`, `ci_low/ci_high` (2.5/97.5 percentiles), and `significant = not (ci_low <= 0 <= ci_high)`. Wired as `--compare-tags A,B` in score-only mode; `compare_hyp_files` aligns rows by `en_text`.
3. **Report fingerprints (P0-3)**. New `src/evaluation/fingerprint.py`: `build_config_fingerprint` records model_id, adapters (or 'base'), max_in/max_new, num_beams, vocab_size, decode device/dtype, seed, mbr config. Every report writes this plus a per-suite `hyp_file_sha256`. `check_resume_fingerprint` raises `FingerprintMismatchError` when the hyp file exists but there is no recorded config, an unreadable/missing report, or a config mismatch; `--force-resume` overrides. Old-scheme reports carry no fingerprint, so resume against them refuses by default (that is exactly the stale-mix risk). `--score-only` now slices to the first `--max-pairs` rows consistent with the report.
4. **Seed MBR (P1-4)**. `mbr_decode` gains `DEFAULT_SEED=12345` and a `seed` param; `set_seed(seed)` is called once per suite before the MBR sampling loop in `translate_pairs`, and `translate_batch_mbr(seed=...)` re-seeds for direct calls. The seed is recorded in the report for MBR and non-MBR runs.
5. **Length ratio + TER (P1-5)**. `score_pairs` now emits `len_ratio = sys_len / ref_len` (from the BLEU result) and `ter` (sacreBLEU TER, lower is better) per suite, plus `entities` and `ref_cleaned`. This makes verbosity visible instead of hidden by BLEU's missing brevity penalty (sys > ref gives bp 1.0) and chrF++'s lack of any length penalty.
6. **Ref-cleaned score (P1-6)**. New `src/evaluation/ref_cleaner.py`: `clean_ref` removes exactly three artifact families from references only -- danda run before ASCII punctuation (`[।॥]+\s*(?=[,.;:!?])`), `#` / `#.` markers, and leading 1-2 digit list markers (`^\s*\d{1,2}(?:[.\u0964\u0965]\s*|\s)`) -- then collapses running spaces. Hypotheses and sources are never touched; `ref_cleaned` is a labeled additive column, never the primary metric. Word-substitution typo normalization (e.g. `क्षेत्रिय` vs `क्षेत्रीय`) is deliberately out of scope: it risks changing meaning with no reliable rule.
7. **Entity/glossary panel (P2-7)**. New `src/evaluation/entity_panel.py`. Entities are canonicalized keys so cross-script pairs match: probe ids come from `LEGAL_PROBES_HI` / `LEGAL_PROBES_EN` (tokenizer bench), EN words matched by word-boundary (`\bterm s?\b`), HI by substring; Article/section numbers map to `('cite', 'article'|'section', int)` across scripts (`Article 227` == `अनुच्छेद 227`, `Section 227` == `धारा 227`); case citations (`S.C.R.`/`INSC`/`AIR`/`SCC`/`SCR`) normalize to period-stripped uppercase tokens; dates normalize to `YYYY-MM-DD` for numeric (`dd/mm/yyyy` tried first) and month-word forms (English + Hindi month map). A wrong Article/Section mapping deliberately scores a miss. Panel aggregates total recall = matched/ref_entities, precision = matched/hyp_entities, F1, over all pairs per suite. Recall-oriented and approximate by design.
8. **Report surface.** `run()` now returns `decode_dtype`, `vocab_size`, `seed`, `bootstrap_samples`, `fingerprint`, and each suite carries `ter`, `len_ratio`, `ref_cleaned`, `entities`, `confidence`, `hyp_file_sha256`.

**What was deliberately scoped out:** re-decoding or re-scoring any real data, editing historical score tables, reference-free QE (COMET-QE) as a metric column, and word-level OCR typo normalization. `eval_legal_mt.py` (deprecated C1) is untouched beyond inheriting the additive `score_pairs` fields.

**Verification:** `make lint` clean; new tests `test_metrics_mt.py` (CIs, paired difference CI, TER, len_ratio, ref-cleaned), `test_ref_cleaner.py`, `test_entity_panel.py`, `test_fingerprint.py` (resume-refusal on mismatch), `test_comet_cache.py` (cache invalidation on hyp change with a fake comet module), `test_mbr_decode.py` (seed reproducibility with a fake model), `test_zero_shot_nllb.py` (score-only max_pairs + compare alignment). Full suite green.
