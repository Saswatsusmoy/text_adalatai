"""Stage A curriculum subsamples + Stage B anti-forget replay mixes."""
...
def build_subsample(
    ...
    if not src_path.exists():
        raise FileNotFoundError(f'{src_path} missing; run make external-eval-split first')
    ...
    if curriculum == 'smoke':
        chosen = _take_up_to(all_rows, int(sub.get('smoke_n', 2000)), rng)
        ...
        if len(chosen) < max_n:
            ...
            leftover = [r for rows in by_src.values() for r in rows if id(r) not in chosen_ids]
            chosen.extend(_take_up_to(leftover, max_n - len(chosen), rng))
        if len(chosen) > max_n:
...
def build_stage_b_replay_mix(
    ...
    if not replay_path.exists():
        a2 = Path('data/external/parallel/subsamples/stage_a_A2_n150000.jsonl')
        if a2.exists():
            replay_path = a2
        else:
            raise FileNotFoundError(f'replay pool missing: {replay_path}; build A2 subsample first')
    ...
    pool_rows = [
        r
        for r in load_jsonl(replay_path)
    ...
    n_replay = max(
        0,
        min(
            int(round(n_assign * (1.0 - assignment_frac) / assignment_frac)),
            len(pool_rows),
        ),
    )
    ...
    actual = role_counts['assignment'] / len(mixed) if mixed else 0.0
    manifest = {
        'curriculum': 'Bp',
        'tag': tag,
        'seed': seed,
        'assignment_frac_target': assignment_frac,
        'assignment_frac_actual': round(actual, 4),
    ...
    if verbose:
        print(
            ...
            f'(assign={n_assign}, replay={len(replay_rows)}, frac={actual:.3f}) '
            f'-> {out_path}'
        )
        print(f'Manifest: {man_path}')
    return manifest
...
def main():
    ...
    p = argparse.ArgumentParser(description='Stage A subsample or Stage B replay mix')
    p.add_argument(
        '--curriculum',
        default='A1',
    ...
    a = p.parse_args()
    if a.curriculum == 'Bp':
        build_stage_b_replay_mix(config_path=a.config)
    else:
        build_subsample(curriculum=a.curriculum, config_path=a.config)
