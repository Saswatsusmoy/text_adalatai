...
def reextract_single(doc_id: int, backend: str = "tesseract", verbose: bool = True) -> dict | None:
    ...
    if verbose:
        ...
        print(f"  [ {status:20s}] Doc {doc_id:2d}: {dev_count:5d} Devanagari chars, {len(text):6d} total chars")
    return {"doc_id": doc_id, "devanagari_chars": dev_count, "total_chars": len(text), "output": str(out_path)}
...
def compare_with_originals(verbose: bool = True) -> dict:
    ...
    for doc_id in CORRUPTED_DOC_IDS:
        ...
        if old_stats and new_stats:
            ...
            if verbose:
                print(f"  Doc {doc_id:2d}: old={old_stats['size_bytes']:>6}B dev={old_stats['devanagari_chars']:>4d}  "
...
def compare_all_with_originals(verbose: bool = True) -> dict:
    ...
    for doc_id in DOC_IDS:
        ...
        if clean_file.exists():
            ...
            results["diffs"].append({"doc_id": doc_id, "old_dev": old_dev, "new_dev": new_dev, "diff": diff})
            if verbose:
                label = "RECOVERED" if old_dev == 0 and new_dev > 0 else "MATCH" if abs(diff) < 500 else "DIFF"
                print(f"  [{label:10s}] Doc {doc_id:2d}: clean={old_dev:5d} dev -> OCR={new_dev:5d} dev (diff={diff:+5d})")
        else:
...
def scan_all_hindi_pdfs(backend: str = "tesseract", verbose: bool = True) -> dict:
    ...
    for doc_id in DOC_IDS:
        ...
        dev_count = count_devanagari(text)
        if dev_count == 0:
            issues.append({"doc_id": doc_id, "issue": "no Devanagari in extraction", "text_preview": text[:100]})
            if verbose:
            ...
            if old_dev == 0 and dev_count > 0:
                ...
                if verbose:
                    print(f"  [CORRUPTED] Doc {doc_id}: clean file has {old_dev} Devanagari, PDF has {dev_count}")
        else:
            if verbose:
...
def main():
    ...
    parser.add_argument(
        "--backend", choices=list(BACKENDS.keys()), default="tesseract",
