...
PROPER_NOUNS = {
    "Act", "Allahabad", "Appellant", "Appellate", "Authority",
    "Bank",
    "Civil", "Commission", "Consolidation", "Constitution", "Court",
    "Development", "Directors", "Disciplinary", "District",
    "Government",
    "High",
    "Land",
    "Malkhan",
    "No.2",
    "Officer",
    "Pandey", "Part", "Petition",
    "Rajbali", "Reference", "Respondent", "Respondents", "Revenue", "Rules",
    "Section", "Sections", "Service", "State",
}
...
def process_doc(doc_id: int, verbose: bool = False) -> dict | None:
    ...
    orig_lines = len([l for l in text.split('\n') if l.strip()])
    new_lines = len([l for l in joined.split('\n') if l.strip()])

    if verbose:
        print(f"  Doc {doc_id:2d}: {orig_lines:4d} -> {new_lines:4d} lines (Δ={orig_lines - new_lines:4d})")
