...
from src.config import HI_PREPROCESSED_DIR, CORRUPTED_DOC_IDS, HI_ORIGINAL_DIR, HI_CLEAN_DIR
...
from src.utils.validation import count_devanagari, has_devanagari, devanagari_ratio
...
class TestExtraction:
    ...
    def test_compare_shows_applied(self):
        ...
        for doc_id, diff in comparison.items():
            assert diff["new_devanagari"] > 0, f"Doc {doc_id}: new file has {diff['new_devanagari']} Devanagari chars (expected >0)"
            assert diff["old_devanagari"] > 0, f"Doc {doc_id}: clean file should have Devanagari after apply"
    ...
    def test_scan_all_pdfs(self):
        ...
        corruption_issues = [i for i in result["issues"] if i.get("issue", "").startswith("corrupted")]
        assert len(corruption_issues) == 0, f"Expected 0 corruption issues after fix, got {len(corruption_issues)}"
...
class TestApply:
    def test_apply_copies_files(self):
        ...
        for doc_id in CORRUPTED_DOC_IDS:
            ...
            assert has_devanagari(clean_text), f"Doc {doc_id}: clean file still has no Devanagari after apply"
