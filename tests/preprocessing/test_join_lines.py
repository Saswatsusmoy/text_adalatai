from src.config import EN_CLEAN_DIR, EN_PREPROCESSED_DIR
from src.preprocessing.join_lines import PROPER_NOUNS, join_lines, process_doc, run, should_join
    ...
    def test_hard_wrapped_docs_reduced(self):
        for doc_id in [1, 2, 6, 7, 8, 10, 11, 12, 14, 17, 21, 25, 26, 27, 28, 29, 30]:
            ...
            clean_lines = len(
                [
                    line
                    for line in clean_path.read_text(encoding='utf-8').split('\n')
                    if line.strip()
                ]
            )
            prep_lines = len(
                [line for line in prep_path.read_text(encoding='utf-8').split('\n') if line.strip()]
            )
